  ...
  <main>
    <div class="mb-4">
      <div class="flex flex-row flex-start">
        ...
        <button
          class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded my-2 mx-1"
          @click="search"
        >
          {{ translations.search }}
        </button>
        <button
          class="inline-flex bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded ml-auto my-2"
          @click="download"
        >
          {{ translations.download_csv }}
          <i class="material-icons-outlined ml-1" aria-hidden="true"
            >download</i
          >
        </button>
      </div>
      <p v-if="variableData && !loading">
        {{ translations.no_of_rows }}: {{ variableData.length }}
      </p>
    </div>

    <Table
      v-if="variableData && !loading"
...
import Table from "../components/table/ElvisTable.vue";
